...
const Doctors = () => {
  ...
  const { speciality } = useParams() // extracted from URL

  const [filterDoc, setFilterDoc] = useState([]) // Stores the filtered list of doctors based on speciality
  const [showFilter, setShowFilter] = useState(false) // Controls the visibility of filter options in mobile view
  const navigate = useNavigate();

  const { doctors } = useContext(AppContext)

  // filters doctors based on speciality
  const applyFilter = () => {
    if (speciality) {
  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        {/* filter button for mobile view - toggles filter visibility */}
